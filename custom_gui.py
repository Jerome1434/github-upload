...
#        self.AmpObj.lp_smooth()
    ...
    def face_Align(self):
        ...
        if self.objectsReady(2):
            ...
            self.face_alCont.confirmloc.clicked.connect(self.face_Align_main)
        else:
            show_message("Must be at least 2 object loaded to run align")
            
    def face_Align_main(self):
            
            # while self.face_alCont.toplocButton.Q
        print('confirmed')
        X = np.zeros(6)
        ...
        render = [self.files[mask].actor, self.files[face].actor]
        self.renWin.renderActors(render)
        
        if 'small' in str(self.face_alCont.mask.currentText()):
            idx = [552, 583, 733, 1099]
        elif 'medium' in str(self.face_alCont.mask.currentText()):
            idx = [130, 297, 546, 1951]
        elif 'large' in str(self.face_alCont.mask.currentText()):
            idx = [271, 273, 704, 898]
        
        
        maskmean = np.mean(self.files[mask].vert[idx,:], axis=0)
        facemean = np.mean(self.files[face].vert[self.face_alCont.face.pnt,:], axis=0)
        X[3:] = maskmean - facemean
        self.files[face].translate(X[3:])
        self.files[face].tform.Translate(X[3:])
        self.renWin.Render()
        
        # vector1a = self.files[mask].vert[idx[1],:] - self.files[mask].vert[idx[0],:]
        # vector1a = vector1a/np.linalg.norm(vector1a)
        # vector1b = self.files[mask].vert[idx[1],:] - self.files[mask].vert[idx[2],:]
        # vector1b = vector1b/np.linalg.norm(vector1b)
        # vector2a = self.files[face].vert[self.face_alCont.face.pnt[1],:] - self.files[face].vert[self.face_alCont.face.pnt[0],:]
        # vector2a = vector2a/np.linalg.norm(vector2a)
        # vector2b = self.files[face].vert[self.face_alCont.face.pnt[1],:] - self.files[face].vert[self.face_alCont.face.pnt[2],:]
        # vector2b = vector2b/np.linalg.norm(vector2b)
        # normal1 = np.cross(vector1a, vector1b)
        # normal1 = normal1/np.linalg.norm(normal1)
        # normal2 = np.cross(vector2a, vector2b)
        # normal2 = normal2/np.linalg.norm(normal2)
        # angx = np.arccos(np.dot(normal2[[1,2]], normal1[[1,2]])/(np.linalg.norm(normal1[[1,2]])*np.linalg.norm(normal2[[1,2]])))
        # angy = np.arccos(np.dot(normal2[[0,2]], normal1[[0,2]])/(np.linalg.norm(normal1[[0,2]])*np.linalg.norm(normal2[[0,2]])))
        # angz = np.arccos(np.dot(normal2[[0,1]], normal1[[0,1]])/(np.linalg.norm(normal1[[0,1]])*np.linalg.norm(normal2[[0,2]])))
        # angz = 0
        vector1 = self.files[mask].vert[idx[0],:] - self.files[mask].vert[idx[1],:]
        vector2 = self.files[face].vert[self.face_alCont.face.pnt[0],:] - self.files[face].vert[self.face_alCont.face.pnt[1],:]
        angx = np.arccos(np.dot(vector2[[1,2]], vector1[[1,2]])/(np.linalg.norm(vector2[[1,2]])*np.linalg.norm(vector1[[1,2]])))
        angz = np.arccos(np.dot(vector2[[0,1]], vector1[[0,1]])/(np.linalg.norm(vector2[[0,1]])*np.linalg.norm(vector1[[0,1]])))
        vector1 = self.files[mask].vert[idx[2],:] - self.files[mask].vert[idx[3],:]
        vector2 = self.files[face].vert[self.face_alCont.face.pnt[2],:] - self.files[face].vert[self.face_alCont.face.pnt[3],:]
        angy = np.arccos(np.dot(vector2[[0,2]], vector1[[0,2]])/(np.linalg.norm(vector2[[0,2]])*np.linalg.norm(vector1[[0,2]])))
        ...
        vtkmatrix = np.eye(4,4)
        vtkmatrix[:3,:3] = R
        vtkmatrix[:3,3] = T
        print(vtkmatrix)
        vtkmatrix4x4 = vtk.vtkMatrix4x4()
        vtkmatrix4x4.Identity()
        for i in range(len(vtkmatrix)):
            for j in range(len(vtkmatrix)):
                    vtkmatrix4x4.SetElement(i,j,vtkmatrix[i,j])
        
        self.files[face].rigidTransform(R, T)
        self.files[face].tform.SetMatrix(vtkmatrix4x4)
        # self.files[face].tform.RotateX(angx*(180/np.pi))
        # self.renWin.Render()
        # self.files[face].tform.RotateZ(angz*(180/np.pi))
        # self.renWin.Render()
        # self.files[face].tform.RotateY(angy*(180/np.pi))
        # self.renWin.Render()
        
        # self.files[face].tform.Translate(T)
        self.renWin.Render()
        print(R)
        print(T)

    
    def face_optDistinit(self, X):
        
        #get the mask, and corresponding vertex id's
        #small
        #topindices = [552]
        #bottomindices = [583]
        #leftindices = [733]
        ...
        moved = np.dot(mv, R.T)
        # moved += X[3:]
        # print(self.face_alCont.face.pnt)
        dist = (moved[self.face_alCont.face.pnt] - self.files[mask].vert[idx])**2
