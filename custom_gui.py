...
#        self.AmpObj.lp_smooth()
    ...
    def face_Align(self):
        ...
        if self.objectsReady(2):
            ...
            self.face_alCont.facedepthICP.clicked.connect(self.face_depthICP)
            self.face_alCont.faceregistration.clicked.connect(self.face_registration)
            self.face_alCont.facedepthregistration.clicked.connect(self.face_depthregistration)
    ...
    def face_deloutlyers(self):
        '''
        distrange :
            the maximum distance from a face vertex to any point on the mask.
            All vertices outside this range will be removed
        ...
        if roughName in fileNames:
            ...
            for i in range(len(dist)):
                if np.absolute(dist[i]) < distrange:
                    faceid.append(i)
    ...
    def face_ICP(self):
        ...
        if face_prepared in fileNames:
            face_al = align(self.files[face_prepared], self.files[mask],
                            maxiter=20, method='linPoint2Plane')
            # print(dir(face_al))
            ...
            # self.fileManager.addRow(alOutput, self.files[alOutput])
            if hasattr(self, 'alCont'):
                self.alCont.getNames()
            if hasattr(self, 'regCont'):
                self.regCont.getNames()
        else:
            show_message("Please run the remove regions outside bounds algorithm")
    
    def face_depthICP(self):
        
        face = str(self.face_alCont.face.currentText())
        mask = str(self.face_alCont.mask.currentText())
        face_prepared = face + '_prepared'
        face_zcorrect = self.files[face_prepared]
        fileNames = []
        for i in range(self.fileManager.n):
            fileNames.append(self.fileManager.getRow(i)[0])
        if face_prepared in fileNames:
            ...
            while correct > 0:
                print(correct)
                face_kdTree = spatial.cKDTree(face_zcorrect.vert)
                [t, correct]  = ampscanGUI.face_normDist(face_kdTree, self.files[mask])
                face_zcorrect.translate(t)
                face_zcorrect.tform.Translate(t)
                self.renWin.Render()
                
                face_al = align(face_zcorrect, self.files[mask],
                                maxiter=5, method='linPoint2Plane')
                # print(dir(face_al))
                face_zcorrect.rigidTransform(face_al.R, face_al.T)
                face_kdTree = spatial.cKDTree(face_zcorrect.vert)
                [t, correct]  = ampscanGUI.face_normDist(face_kdTree, self.files[mask])
                ...
                face_al.m.actor.SetUserTransform(face_al.m.tform)
            alName = face+'_al'
            depthalName = face+'_depthal'
            self.files[depthalName] = face_al.m
            self.filesDrop.append(depthalName)
            self.fileManager.addRow(depthalName, self.files[depthalName])
            self.fileManager.setTable(mask, [1,0,0], 1, 2)
            self.fileManager.setTable(face, [1,1,1], 1, 0)
            self.fileManager.setTable(face_prepared, [0,0,1], 1, 0)
            self.fileManager.setTable(alName, [0,0,1], 1, 0)
            self.fileManager.setTable(depthalName, [0,0,1], 1, 2)
            # self.fileManager.addRow(alOutput, self.files[alOutput])
            if hasattr(self, 'alCont'):
                self.alCont.getNames()
            if hasattr(self, 'regCont'):
    ...
    def face_normDist(face, mask):
        ...
        """
        mask.calcVNorm()
        [dist, idx] = face.query(mask.vert, 1)
        D = face.data[idx] - mask.vert
        n = mask.vNorm
        values = np.linalg.norm(D, axis=1)
        polarity = np.sum(n*D, axis=1) < 0
        values[polarity] *= -1.0
        maxid = np.argmax(values)
        maxdist = face.data[idx][maxid] - mask.vert[maxid]
        return [maxdist, values[maxid]]
    ...
    def face_registration(self):
        ...
        if face_al in fileNames:
            ...
            self.fileManager.setTable(mask, [1,0,0], 1, 0)
            self.fileManager.setTable(face_al, [0,0,1], 1, 0)
            
            face_reg = registration(self.files[mask], self.files[face_al], steps = 1,
                               smooth=0).reg
            ...
            if hasattr(self, 'regCont'):
                self.regCont.getNames()
            #im = []
            face_reg.actor.setScalarRange([-10,10])
    ...
    def face_depthregistration(self):
        
        mask = str(self.face_alCont.mask.currentText())
        face = str(self.face_alCont.face.currentText())
        face_depthal = str(self.face_alCont.face.currentText())+'_depthal'
        
        fileNames = []
        for i in range(self.fileManager.n):
            fileNames.append(self.fileManager.getRow(i)[0])
        if face_depthal in fileNames:
            c1 = [31.0, 73.0, 125.0]
            c3 = [170.0, 75.0, 65.0]
            c2 = [212.0, 221.0, 225.0]
            CMap1 = np.c_[[np.linspace(st, en) for (st, en) in zip(c1, c2)]]
            ...
            self.fileManager.setTable(mask, [1,0,0], 1, 0)
            self.fileManager.setTable(face_depthal, [0,0,1], 1, 0)
            
            face_reg = registration(self.files[mask], self.files[face_depthal], steps = 1,
                               smooth=0).reg
            ...
            #reg.addActor(CMap = self.CMap02P)
            face_reg.addActor(CMap = self.CMapN2P)
            depthregName = face + '_depthreg'
            self.files[depthregName] = face_reg
            self.filesDrop.append(depthregName)
            self.fileManager.addRow(depthregName, self.files[depthregName])
            if hasattr(self, 'alCont'):
                self.alCont.getNames()
            if hasattr(self, 'regCont'):
                self.regCont.getNames()
            ...
            self.face_alCont.consolewidget.append('Run the Registration code between %s and %s' % (mask, face_depthal))
    ...
    def face_CMapOut(self, amp, colors):
        ...
        im = ampscanGUI.face_genIm(amp, size=[3200, 8000], views=[[0, 0, 1]],crop=True)[0]
        axes[0].imshow(im)
        axes[0].set_title('Anterior')
        axes[0].set_axis_off()
        #plt.colorbar(CMap)
        c1 = [31.0, 73.0, 125.0]
        c3 = [170.0, 75.0, 65.0]
        ...
        N, bins, patches = axes[2].hist(amp.values, bins=11, range=(-10, 10), density=True)
        axes[2].set_xlabel('Shape deviation, mm')
        axes[2].set_ylabel('Probability density')
        
        for i, p in enumerate(patches):
            p.set_facecolor(CMap[:, i])
        axes[2].set_title('Histogram of the distance [mm]')
        fig.set_size_inches([9, 4])
        mask = str(self.face_alCont.mask.currentText())
        face = str(self.face_alCont.face.currentText())
        if 'small' in mask:
            filename = face+'_reg_small.png'
        if 'medium' in mask:
            filename = face+'_reg_medium.png'
        if 'large' in mask:
            filename = face+'_reg_large.png'
    ...
    def face_alOut(self):
        ...
        for i in range(self.fileManager.n):
            fileNames.append(self.fileManager.getRow(i)[0])
            if (fileNames[i] == face_al or
                fileNames[i] == mask):
                self.fileManager.table.item(i, 4).setCheckState(2)
                # self.fileManager.setTable(fileNames[i], [1,0,0], 1, 2)
            else:
                self.fileManager.table.item(i, 4).setCheckState(0)
                # self.fileManager.setTable(fileNames[i], [1,1,1], 1, 0)
        
        fig,axes = plt.subplots(ncols=2)
        im = ampscanGUI.face_genIm(self.files[face_al], self.files[mask],size=[3200, 8000], views=[[0, 0, 1]],crop=True)[0]
        # im = ampscanGUI.face_genIm(self.files[alOutput], size=[3200, 8000], views=[[0, 0, 1]],crop=True)[0]
        axes[0].imshow(im)
        axes[0].set_title('Anterior')
        axes[0].set_axis_off()
        ...
        if 'small' in mask:
            filename = face_al+'_small.png'
        if 'medium' in mask:
            filename = face_al+'_medium.png'
        if 'large' in mask:
            filename = face_al+'_large.png'
        plt.savefig(os.getcwd()+'\\output\\'+filename, dpi=600)
    
    def face_genIm(amp, amp2 = None, size=[512, 512], views=[[0, 0, 1]], 
        ...
        """
        if not hasattr(amp, 'actor'):
            amp.addActor()
        if amp2 != None:
            if not hasattr(amp2, 'actor'):
                amp2.addActor()
...
#            win.addAxes([self.actor,], color=[0.0, 0.0, 0.0], viewport=i)
            # win.setView(view, i)
            cam = win.rens[i].GetActiveCamera()
            cam.Elevation(0)
            cam.SetPosition(view[0], view[1], view[2])
#            win.setProjection(projection, viewport=i)
            win.renderActors([amp.actor,], zoom=zoom)
            if amp2 != None:
                win.renderActors([amp.actor, amp2.actor], zoom=zoom)
...
class Face_AlignControls(QMainWindow):
    ...
    def __init__(self, names, parent = None):
        ...
        self.facedepthICP = QPushButton('run depthICP')
        self.faceregistration = QPushButton('run registration')
        self.facedepthregistration = QPushButton('run depth registration')
        ...
        vbox1.addWidget(self.facedepthICP, 8)
        vbox1.addWidget(self.faceregistration, 9)
        vbox1.addWidget(self.facedepthregistration, 10)
