...
#        qs[4:, 0] = Ts[:, 0]
    ...
    def optPoint2Point(mv, kdTree, opt='L-BFGS-B'):
        ...
        Xres = np.reshape(X1[2][1],[1,Ns**n])
        Yres = np.reshape(X1[2][2],[1,Ns**n])
        Zres = np.reshape(X1[2][3],[1,Ns**n])
        res = np.reshape(X1[3],[1,Ns**n])
        X2 = np.array([res[0,:], Xrotres[0,:], Xres[0,:], Yres[0,:], 
                       Zres[0,:]])
        ...
        Xfinal = np.array([Xfinal.x[0], 0, 0, Xfinal.x[1], Xfinal.x[2], 
                           Xfinal.x[3]])
    ...
    def optDistError(X, mv, kdTree, out='err'):
        ...
        [angx, angy, angz] = np.array([X[0], 0, 0])
        Rx = np.array([[1, 0, 0],
                        [0, np.cos(angx), -np.sin(angx)],
                        [0, np.sin(angx), np.cos(angx)]])
        Ry = np.array([[np.cos(angy), 0, np.sin(angy)],
        ...
        R = np.dot(np.dot(Rz, Ry), Rx)
        moved = np.dot(mv, R.T)
        moved = moved + np.array([X[1], X[2], X[3]])
